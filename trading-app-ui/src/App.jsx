  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col p-4">
        ...
        <nav className="flex flex-col gap-2 flex-1">
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition ${
              activeTab === 'history' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition ${
              activeTab === 'rules' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
      ...
      <main className="flex-1 flex flex-col bg-slate-950 overflow-hidden">
          ...
          <div className="flex-1 flex overflow-hidden">
            <div className="flex-1 flex flex-col">
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {chatLogs.length === 0 ? (
                  <div className="text-center text-slate-500 my-auto pt-20">
                    <p>No trade audits yet. Enter a signal below to analyze against your strategy rules.</p>
                  </div>
                ) : (
                  chatLogs.map((log) => (
                    <div
              ...
              {/* Chat Input */}
              <form onSubmit={handleSendAudit} className="p-4 border-t border-slate-800 bg-slate-900/40 flex gap-3">
                <input
                  type="text"
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  placeholder="Type signal (e.g. Buy EURUSD at 1.0850, SL 1.0820, TP 1.0940)..."
                  className="flex-1 bg-slate-900 border border-slate-800 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500"
                />
                <button
                  type="submit"
                  className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition"
                >
            ...
            <aside className="w-80 border-l border-slate-800 p-6 bg-slate-900/30 overflow-y-auto">
              <h3 className="text-md font-bold text-white mb-4">Audit Inspector</h3>
              {selectedAudit ? (
        ...
        {activeTab === 'history' && (
          <div className="flex-1 p-6 overflow-y-auto">
            ...
            {historyLogs.length === 0 ? (
              <p className="text-slate-500 text-sm">No historical trade records found in Supabase.</p>
            ) : (
              <div className="grid gap-4">
                {historyLogs.map((log) => {
                  ...
                  return (
                    <div key={log.id} className="p-4 bg-slate-900 border border-slate-800 rounded-xl space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-indigo-400 text-sm">{log.asset_pair || 'CHAT_SIGNAL'}</span>
        ...
        {activeTab === 'rules' && (
          <div className="flex-1 p-6 overflow-y-auto">
            <h3 className="text-lg font-bold text-white mb-2">Strategy Rules & Knowledge Base</h3>
            ...
            <div className="p-5 bg-slate-900 border border-slate-800 rounded-xl space-y-3">
              <h4 className="font-semibold text-indigo-400 text-sm">Active RAG Integration</h4>
